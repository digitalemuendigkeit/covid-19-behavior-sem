---
title: "COVID-19 Model Iteration 3-b-2 with copula Measurement Model Evaluation"
...
rapre <- readRDS("Models/rapre-co-3-b-2.RDS")
raprc <- readRDS("Models/raprc-co-3-b-2.RDS")
radn <- readRDS("Models/radn-co-3-b-2.RDS")
rabi <- readRDS("Models/rabi-co-3-b-2.RDS")
...
forevalci <- bind_rows(cvpre, cvprc, cvdn, cvbi)
forevalbase <- evalmm %>%
  filter(Mode == "B") %>%
...
forhocevalbase <- evalmm %>%
  filter(Mode == "HOCB") %>% 
  rename(HOC = Construct, 
         LOC = Indicator.or.LOC) %>% 
  select(!c(Type, Mode))
```
...
write_excel_csv(refevalmm %>% mutate(across(3:7,  round, 2)), "mmevalref-co-3-b-2-cop.csv",delim = ";")
write_excel_csv(boothtmt %>% mutate(across(3:4,  round, 2)), "htmt-co-3-b-2-cop.csv",delim = ";")
write_excel_csv(forevalmm %>% mutate(across(c(3:8, 10:12),  round, 2)), "mmevalfor-co-3-b-2-cop.csv",delim = ";")
write_excel_csv(forhocevalmm %>% mutate(across(c(3:6, 8:10),  round, 2)), "mmevalhoc-co-3-b-2-cop.csv",delim = ";")
```

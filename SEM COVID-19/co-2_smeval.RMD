...
Compentence has an effect on all its descendent lower-order constructs except Perceived Susceptibility.
Integrity has no effect on any of its descendent lower order constructs.
...
With lower-order constructs, insignificant paths are those from Knowledge to Perceived Response Costs, all from Benevolence except those from Perceived Response Costs and Perceived Severity, the path from Competence to Perceived Susceptibility,  all from Integrity except to Perceived Response Efficacy, all paths from the Response and Threat Beliefs lower-order constructs except the one from Perceived Response Efficacy and the path from Injunctive Norm to Behavioral Intention.
...
Concerning the remaining lower-order constructs, Benevolence only has a significant path with a discernible effect to Perceived Response Costs and so does Integrity, although with only a negligible effect.
Knowledge has no discernible effect on and no significant path to Perceived Response Costs.
Only Perceived Response Efficacy has a significant path to and an effect on Behavioral Intention, for Perceived Response Costs the path is on the edge of significance with a discernible effect.
Perceived Self-Efficacy has neither.
Therefore, Perceived Self-Efficacy should be removed as a lower-order construct.
Injunctive Norm has neither a significant path to nor a discernible effect on Behavioral Intention.

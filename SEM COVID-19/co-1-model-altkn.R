...
# Alternative measures for knowledge
...
data$'COKNCo' <- rowSums(data %>% select(contains("COKN")) == 3)
data$'COKNDk' <- rowSums(data %>% select(contains("COKN")) == 2)
data$'COKNIn' <- rowSums(data %>% select(contains("COKN")) == 1)
...
mm <- constructs(
  ...
  composite("Knowledge Correct", single_item("COKNCo")),
  composite("Knowledge Incorrect", single_item("COKNIn")),
  composite("Knowledge Don't Know", single_item("COKNDk")),
...
sm <- relationships(
  paths(from = c("Distrusting Beliefs", "Knowledge Correct", "Knowledge Incorrect", "Knowledge Don't Know"), to = c("Response Beliefs", "Threat Beliefs")),
...
summo <- summary(model)
saveRDS(model, "SEM COVID-19/Models/model-co-1-altkn.RDS")
#model <- readRDS("SEM COVID-19/Models/model-co-1.RDS")
#plot(model)

# summo <- summary(model)
# sumfs <- summary(model$first_stage_model)
bootmodel <- bootstrap_model(model, nboot = 500)
sumbo <- summary(bootmodel)
saveRDS(bootmodel, "SEM COVID-19/Models/model-boot-co-1-altkn.RDS")

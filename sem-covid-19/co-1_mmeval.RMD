...
```{r indicator deletion personal norm, echo = FALSE}
unlist(summo$descriptives$correlations$items)[model$mmMatrix[33:35, 2], model$mmMatrix[33:35, 2]] %>%
  data.frame() %>%
  datatable(caption = 'Personal Moral Norm Item Intercorrelations',
...
```{r ra6, echo =FALSE}
plot(all_reduds$model[[6]], 
     title = all_reduds$title[[6]])
```
...
The weight for Distrusting Beliefs Integrity is not significant, but the loading is sufficiently large.

---
title: "COVID-19 Model Iteration 3-b-1 Structural Model Evaluation"
output: 
...
# Load PLS Model
model <- readRDS("Models/model-co-3-b-1.RDS")
bootmodel <- readRDS("Models/model-boot-co-3-b-1.RDS")
# Load bootstrapped first stage PLS model
bootfsmodel <- readRDS("Models/model-fs-boot-co-3-b-1.RDS")
# Load approximation PLS model (no HOCs, data formatted as data.frame)
proxymodel <- readRDS("Models/model-proxy-co-3-b-1.RDS")
summo <- summary(model)
sumfs <- summary(model$first_stage_model)
sumbomo <- summary(bootmodel)
sumbofsmo <- summary(bootfsmodel)
# Set seminr plot theme
thm <- seminr_theme_create(construct.compositeA.arrow = "backward", construct.compositeA.use_weights = FALSE, plot.adj = FALSE, construct.compositeA.shape = "ellipse", construct.compositeB.shape = "ellipse")
...
Integrity has no effect on any of its descendant lower order constructs.
Knowledge has only a negligible effect on Perceived Response Costs.
...
The only insignificant paths are those from Knowledge and Integrity to Perceived Response Costs and from Benevolence and Integrity to Perceived Response Efficacy.
...
All other construct and higher-order construct paths are significant with an acceptable effect size.

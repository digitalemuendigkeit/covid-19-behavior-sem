...
cormv <- correlation_matrix(df = data[,1:37] %>%
                     select(!c(ends_with("General"), ends_with("Mask"), ends_with("App"), ends_with("Contact"))),
                   type = "spearman",
                   use = "lower",
                   digits = 2)
data.frame(Variable = paste(1:17, rownames(cormv))) %>% 
...
## Income
```{r income, echo = FALSE, layout="l-page"}
...
## Knowledge
In this section, we examine the correlation between specific questions and Behavior as well as Subjective Knowledge.

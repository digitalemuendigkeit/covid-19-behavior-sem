...
sm <- relationships(
  paths(from = c("Distrusting Beliefs", "Knowledge"), to = c("Perceived Self-Efficacy", "Perceived Response Efficacy")),
  paths(from = c("Perceived Self-Efficacy", "Perceived Response Efficacy"), to = "Behavioral Intention")
)


# model <- estimate_pls(data, mm, sm)
# saveRDS(model, "SEM Climate Crisis/Models/model-cc-2-a-3-b-1.RDS")
model <- readRDS("SEM Climate Crisis/Models/model-cc-2-a-3-b-1.RDS")
plot(model)
# bootmodel <- bootstrap_model(model, nboot = 5000)
# saveRDS(bootmodel, "SEM Climate Crisis/Models/model-boot-cc-2-a-3-b-1.RDS")
bootmodel <- read_rds("SEM Climate Crisis/Models/model-boot-cc-2-a-3-b-1.RDS")
plot(bootmodel)
# bootfsmodel <-  bootstrap_model(model$first_stage_model, nboot = 5000)
# saveRDS(bootfsmodel, "SEM Climate Crisis/Models/model-fs-boot-cc-2-a-3-b-1.RDS")
...
saveRDS(proxymodel, "SEM Climate Crisis/Models/model-proxy-cc-2-a-3-b-1.RDS")

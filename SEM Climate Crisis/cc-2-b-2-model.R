...
sm <- relationships(
  ...
  paths(from = c("Response Beliefs", "Threat Beliefs", "Personal Moral Norm", "Descriptive Norm"), to = "Behavioral Intention")
)


# model <- estimate_pls(data, mm, sm)
# saveRDS(model, "SEM Climate Crisis/Models/model-cc-2-b-2.RDS")
model <- readRDS("SEM Climate Crisis/Models/model-cc-2-b-2.RDS")
plot(model)
# bootmodel <- bootstrap_model(model, nboot = 5000)
# saveRDS(bootmodel, "SEM Climate Crisis/Models/model-boot-cc-2-b-2.RDS")
bootmodel <- read_rds("SEM Climate Crisis/Models/model-boot-cc-2-b-2.RDS")
plot(bootmodel)
# bootfsmodel <-  bootstrap_model(model$first_stage_model, nboot = 5000)
# saveRDS(bootfsmodel, "SEM Climate Crisis/Models/model-fs-boot-cc-2-b-2.RDS")
...
saveRDS(radn, "SEM Climate Crisis/Models/radn-cc-2-b-2.RDS")
...
proxymm <- constructs(
  ...
  composite("Personal Moral Norm", multi_items("CCPN",c(1,3))),
  composite("Descriptive Norm", c("CCDN1", "CCDN2"), weights = mode_B),
  composite("Behavioral Intention", single_item("CCBI2"))
)
...
saveRDS(proxymodel, "SEM Climate Crisis/Models/model-proxy-cc-2-b-2.RDS")

---
title: "Climate Crisis Model Iteration 2-b-2 (Only Heating) Structural Model Evaluation"
...
model <- readRDS("Models/model-cc-2-b-2.RDS")
bootmodel <- readRDS("Models/model-boot-cc-2-b-2.RDS")
bootfsmodel <- readRDS("Models/model-fs-boot-cc-2-b-2.RDS")
proxymodel <- readRDS("Models/model-proxy-cc-2-b-2.RDS")
summo <- summary(model)
...
```
The VIFs vor many of the relationships between lower-order constructs are above 3, but none is above 5.
...
For Response Beliefs, Perceived Self-Efficacy and Perceived Response Efficacy the model predictiveness is very weak.
For all other constructs the model predictiveness is weak.
...
```
The effect sizes for many of the relationships involving lower-order constructs is below the critical threshold.
Among these, there is no discernible effect of Integrity on any construct.
Competence only has an effect on Perceived Severity, and  Benevolence on Perceived Self-Efficacy and Perceived Severity.
From the lower-order constructs, only Perceived Self-Efficacy has an influence on Behavioral Intention.
Knowledge has only an effect on Threat Beliefs and its lower-order constructs.
Among the (higher-order) constructs, there is no effect of Knowledge on Response Beliefs, or of Threat Beliefs or Descriptive Norm on Behavioral Intention.
...
```
For the approximation model, the PLS model predicts CCIBI2 and CCRB8 (Perceived Response Costs) better than the naive linear model.
...
```
Many of the path coefficients involving lower-order constructs are insignificant.
None of the paths from Integrity is significant, and for Competence only the paths to the Threat Beliefs lower-order constructs.
For Benevolence, all except the one to Perceived Response Costs are significant.
Concerning the Response and Threat Beliefs lower-order constructs, only Perceived Self-Efficacy and Perceived Response Efficacy have significant paths to Behavioral Intention.
Knowledge has significant paths to Threat Beliefs and its lower-order constructs, and to the lower-order construct Perceived Self-Efficacy.
All paths from Distrusting Beliefs are significant.
The paths from Threat Beliefs and Descriptive Norm to Behavioral Intention are insignificant.
...
# Results summary
The path from Knowledge to Response Beliefs is insignificant and shows no effect, as do the paths from Threat Beliefs and Descriptive Norm.

...
bootmodel <- readRDS("Models/model-boot-cc-2.RDS")
summo <- summary(model)
sumfs <- summary(model$first_stage_model)
sumbomo <- summary(bootmodel)
# Load redundancy analysis models
rapse <- readRDS("Models/rapse-cc-2.RDS")
rapre <- readRDS("Models/rapre-cc-2.RDS")
raprc <- readRDS("Models/raprc-cc-2.RDS")
radn <- readRDS("Models/radn-cc-2.RDS")
rain <- readRDS("Models/rain-cc-2.RDS")
rabi <- readRDS("Models/rabi-cc-2.RDS")
...
datatable(refevalmm,
  ...
  formatRound(3:7,
              digits = 3) %>%
  formatStyle('Loading', backgroundColor = styleInterval(cuts = c(0.4, 0.708), values = c('#ffcccc', '#ffe5e5', '#00'))) %>%
  formatStyle('AVE', backgroundColor = styleInterval(cuts = c(0.5), values = c('#ffcccc', '#00'))) %>%
  formatStyle('Cronbach\'s alpha', backgroundColor = styleInterval(cuts = c(0.6,0.9,0.95,0.999), values = c('#ffcccc', '#00','#ffe5e5',  '#ffcccc', '#00'))) %>%
  formatStyle('Composite reliability', backgroundColor = styleInterval(cuts = c(0.6,0.9,0.95,0.999), values = c('#ffcccc', '#00', '#ffe5e5', '#ffcccc', '#00'))) %>%
    formatStyle('Construct reliability', backgroundColor = styleInterval(cuts = c(0.6,0.9,0.95,0.999), values = c('#ffcccc', '#00','#ffe5e5',  '#ffcccc', '#00'))) %>%
...
datatable(boothtmt,
  ...
  formatRound(3:4,
              digits = 3) %>%
  formatStyle('Lower CI HTMT', backgroundColor = styleInterval(cuts = c(0.9,1), values = c('#00', '#ffe5e5', '#ffcccc'))) %>%
  formatStyle('Upper CI HTMT', backgroundColor = styleInterval(cuts = c(0.9,1), values = c('#00', '#ffe5e5', '#ffcccc')))
```

## Results of the evaluation of the reflective measurement model
All indicators show sufficient convergent validity, internal consistency reliability and discriminant validity. The hypothesized reflective measurement model and all its indicators can be retained.
...
There are no issues with collinearity as measured with VIF, and all indicators are significant and relevant. Convergent validity is established for Descriptive Norm and Injunctive Norm.
For all other constructs, convergent validity could not be established. Indicator deletion led to worse or unchanged results:
For Perceived Self-Efficacy, R² is now 0.328 (was 0.328) and the path coefficient is 0.573 (was 0.573).
For Perceived Response Efficacy, R² = 0.353 (was 0.386) and path coefficient = 0.594 (was 0.621).
For Perceived Response Costs, R² = 0.253 (was 0.281) and path coefficient = 0.503 (was 0.530).
For Behavioral Intention, R² = 0.360 (was 0.406) and path coefficient = 0.600 (was 0.637).

```{r redundancy analyis, echo = FALSE, include=FALSE}
# Perceived Self-Efficacy
unlist(sumrapse$descriptives$correlations$items)[rapse$mmMatrix[3,2],rapse$mmMatrix[1:2,2]]
# Perceived Response Efficacy
unlist(sumrapre$descriptives$correlations$items)[rapre$mmMatrix[3,2],rapre$mmMatrix[1:2,2]]
# Perceived Response Costs
unlist(sumraprc$descriptives$correlations$items)[raprc$mmMatrix[3,2],raprc$mmMatrix[1:2,2]]
# Behavioral Intention
unlist(sumrabi$descriptives$correlations$items)[rabi$mmMatrix[3,2],rabi$mmMatrix[1:2,2]]
```
Further indicator deletion is unlikely to improve convergent validity.
...
For the formative higher-order constructs, there are no issues with collinearity and all weights are significant and relevant. The formative higher-order constructs can be retained as hypothesized.

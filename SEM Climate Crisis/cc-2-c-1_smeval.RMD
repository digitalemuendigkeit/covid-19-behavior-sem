---
title: "Climate Crisis Model Iteration 2-c-1 (Driving) Structural Model Evaluation"
...
model <- readRDS("Models/model-cc-2-c-1.RDS")
bootmodel <- readRDS("Models/model-boot-cc-2-c-1.RDS")
bootfsmodel <- readRDS("Models/model-fs-boot-cc-2-c-1.RDS")
proxymodel <- readRDS("Models/model-proxy-cc-2-c-1.RDS")
summo <- summary(model)
...
Benevolence and Competence have only a discernible effect on the Threat Beliefs lower-order constructs.
Additionally, there seems to be no discernible effect of Threat Beliefs, Subjective Norm, or Personal Moral Norm on Behavioral Intention, or of Knowledge on Response Beliefs.
...
```
For the approximation model, only CCBI1 is predicted better by the PLS model than by the linear model in terms of RMSE.
...
```
As already hinted by a lack of effect size, neither Personal Moral Norm nor Subjective Norm or Threat Beliefs show a significant influence on Behavioral Intention as indicated by the path coefficients, nor does Knowledge on Response Beliefs.
Looking at relationships involving lower-order constructs, Knowledge or Integrity have no significant influence on the Response Beliefs lower-constructs, either.
...
# Results summary
The paths from Personal Moral Norm, Subjective Norm, and Threat Beliefs to Behavioral Intention are insignificant and the effect sizes are negligible.
Therefore, those exogenous constructs can be removed from the model.
The path from Knowledge on Response Beliefs is also insignificant and shows no effect.
Therefore, Knowledge can also be removed.

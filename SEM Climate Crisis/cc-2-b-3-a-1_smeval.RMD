---
title: "Climate Crisis Model Iteration 2-b-3-a (Only Heating) Structural Model Evaluation"
...
```
There is no VIF information for Distrusting Beliefs Benevolence.
...
```
In-sample predictive power is only weak for Behavioral Intention.
For all others it is very weak or even negligible.
...
```
The effects of Perceived Response Efficacy and Injunctive Norm are negligible.
The effect of Distrusting Beliefs Benevolence on Response Beliefs and Perceived Response Efficacy is medium, all others effects are small.
...
```
Only CCBI2 is better predicted by the PLS than the benchmark LM.
Overall, the out-of-sample predictive power is very low.
...
```
With the exception of the path from Injunctive Norm and Perceived Response Efficacy on Behavioral Intention, all paths are significant.
...
datatable(sumsm,
  ...
  formatStyle('f²', backgroundColor = styleInterval(cuts = c(0.0), values = c('#ff9999', '#00'))) %>%
  formatStyle('VIF', backgroundColor = styleInterval(cuts = c(3,5), values = c('#00', '#ffe5e5','#ff9999')))
```

# Results summary
The path from Injunctive Norm to Behavioral Intention is non-significant on the basis of the bootstrap CI, but there is an effect.
Perceived Response Efficacy has no significant effect on or path coefficient to Behavioral Intention, however, Response Beliefs does.
All other constructs and paths can be retained.
SO this model can be included in model comparison.
However, Injunctive Norm and Perceived Response Efficacy can be tentatively removed to observe if that increases model predictivity.

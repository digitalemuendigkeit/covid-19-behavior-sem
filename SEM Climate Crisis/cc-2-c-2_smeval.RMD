...
In particular, there is no discernible effect of Integrity or Benevolence on any other lower-order construct.
Competence has only a discernible effect on Perceived Response Efficacy.
...
The effect of Response Beliefs on Behavioral Intention is larger than one.
...
```
For the approximation model, only CCBI3 is predicted better by the PLS model than by the linear model in terms of RMSE.
This indicates that the model has low to no out-of-sample predictive power.
...
```
Benevolence and Competence only have a significant influence on Perceived Response Efficacy, and Integrity has none at all. 
...
# Results summary
All paths involving regular and higher-order constructs are significant and the effect sizes are sufficiently large.
Interestingly, the effect size of Response Beliefs on Behavioral Intention is even larger than one. 
Concerning lower-order constructs, Integrity should be removed from the Distrusting Beliefs HOC.
As both other Competence and Benevolence only have significant paths to (and only Competence has a discernible effect on) Perceived Response Efficacy, one could also try splitting up the Response Beliefs higher-order construct to differentiate the effect of Distrusting Beliefs.

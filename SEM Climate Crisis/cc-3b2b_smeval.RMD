---
title: "Climate Crisis Model 3 b 2 b Structural Model Evaluation"
...
model <- readRDS("Models/model-cc-3b-2b.RDS")
bootmodel <- readRDS("Models/model-boot-cc-3b-2b.RDS")
bootfsmodel <- readRDS("Models/model-fs-boot-cc-3b-2b.RDS")
summo <- summary(model)
...
```
Looks like I broke the code because there is no VIF for the Response Beliefs constructs with Distrusting Beliefs? However, there are VIF values for the Distrusting Beliefs LOCs.
...
```
As expected, the R² for PSE and PRC is very low, while the R² for PRE is slightly higher than the one for Response Beliefs was. This indicates that Distrusting Beliefs mostly influence Response Beliefs, justifying the splitting of the higher-order constructs.
...
```
As expected, there is only a weak influence of Distrusting Beliefs on PSE and PRC. Further looking at the influence of the different Distrusting Beliefs LOCs, it appears that Integrity has the weakest effects overall (all f² below 0.02), while Benevolence mostly has an effect on Perceived Severity, and Competence has an effect on PRE and Perceived Severity.
A possible take-away, if this is supported by the path coefficients, would be to also split the respective higher-order constructs.
...
```
There are several insignificant paths, among them all paths from Integrity except for the one to Perceived Susceptibility (Integrity > Perceived Severity is on the edge with a 0 in the CI but significant t), the paths from Benevolence to Perceived Response Costs and, to a lesser degree (only 0 in CI) to Perceived Self-Efficacy, the paths from Competence to Perceived Self-Efficacy to Perceived Response Costs, and for Perceived Severity and Perceived Susceptibility to Behavioral Intention.
...
# Results summary
In summary, while on the construct level all paths seem to be significant and relevant, and all effects big enough, all HOCs could be split to look for the individual influences.

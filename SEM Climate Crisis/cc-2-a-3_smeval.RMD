---
title: "Climate Crisis Model Iteration 2-a-3 Structural Model Evaluation"
...
```
There are no collinearity issues as indicated by VIF.
...
```
The in-sample predictive power for Response Beliefs is very weak, with R² for Perceived Self-Efficacy being below even that threshold.
...
Again, effect sizes for relationships involving lower-order constructs are very low.
Specifically, there is no discernible effect of Benevolence, Competence or Integrity.
On the other hand, there is a medium effect of Distrusting Beliefs on Response Beliefs.
The effect of Knowledge on Response Beliefs and its lower-order constructs is also negligible.
...
```
Again, only for Behavioral Intention does the model show out-of-sample predictive power.
...
```
All path coefficients not involving lower-order constructs are significant.
Concerning paths with lower-order constructs, Benevolence and Competence have no significant path to either Response Beliefs lower-order construct.
Integrity has no significant path to Perceived Response Efficacy.
...
# Results summary
Overall, the model is satisfactory, but in-sample as well as out-sample predictive power is low and effect sizes, with the exception of the effect of Response Beliefs on Behavioral Intention, are as well.
Competence and Benevolence should be removed from the Distrusting Beliefs higher-order construct because of a lack of effect or significant paths, making Distrusting Belief a Distrusting Belief Integrity standard construct.
...
But this model can also be used as a final point for model comparison.

---
title: "Climate Crisis Model Iteration 3-a Measurement Model Evaluation"
...
radn <- readRDS("Models/radn-cc-3-a.RDS")
rain <- readRDS("Models/rain-cc-3-a.RDS")
...
```{r plot bootmodel, echo=FALSE, out.width=100%}
...
```{r plot mm model, echo=FALSE, out.width=100%}
...
All multi-item indicators and constructs show sufficient convergent validity, internal consistency reliability and discriminant validity. However, respecifying the Response Beliefs LOC as single-indicator reflective constructs leads to problems in discriminant validity respective Personal Moral Norm, with the HTMT upper 95 % confidence bound just clearing 1 (1.033). However, this is likely due to the properties of the single-indicator constructs. Therefore, both reflective constructs can be retained as-is. CCPN2 could be removed.

```{r find cross-loadings, echo = FALSE, include = FALSE}
# Personal Moral Norm
## Intercorrelation
icpm <- evalmm %>% filter(Construct == "Personal Moral Norm") %>%
  cbind(cor(data[,(evalmm %>% filter(Construct == "Personal Moral Norm"))$Indicator.or.LOC]))
## cross-loadings
clpm <- data.frame(Indicator = character(),
                       Construct = character(),
                       Loading = double ())
for (i in 1:length(icpm$Indicator)){
  x <- icpm$Indicator[i]
  clpm <- clpm %>% rbind(data.frame(Indicator =  {{x}},
                                            Construct = names(sumfs$validity$cross_loadings[{{x}},]),
                                            Loading = sumfs$validity$cross_loadings[{{x}},]))
  
}
data.table::setorder(clpm, Loading)
clpm
# Personal Moral Norm has high intercorrelation and some weaker cross-loadings on Perceived Severity, Behavioral Intention and Perceived Self-Efficacy (CCPN2)

# Perceived Self Efficacy
## Intercorrelation -> none, only one indicator
icpse <- evalmm %>% filter(Construct == "Perceived Self-Efficacy")
## cross-loadings
clpse <- data.frame(Indicator = character(),
                       Construct = character(),
                       Loading = double ())
for (i in 1:length(icpse$Indicator)){
  x <- icpse$Indicator[i]
  clpse <- clpse %>% rbind(data.frame(Indicator =  {{x}},
                                            Construct = names(sumfs$validity$cross_loadings[{{x}},]),
                                            Loading = sumfs$validity$cross_loadings[{{x}},]))
  
}
data.table::setorder(clpse, Loading)
clpse
# Some loading on Personal Moral Norm (0.7)

# Perceived Response Efficacy
## Intercorrelation -> none, only one indicator
icpre <- evalmm %>% filter(Construct == "Perceived Response Efficacy")
## cross-loadings
clpre <- data.frame(Indicator = character(),
                       Construct = character(),
                       Loading = double ())
for (i in 1:length(icpre$Indicator)){
  x <- icpre$Indicator[i]
  clpre <- clpre %>% rbind(data.frame(Indicator =  {{x}},
                                            Construct = names(sumfs$validity$cross_loadings[{{x}},]),
                                            Loading = sumfs$validity$cross_loadings[{{x}},]))
  
}
data.table::setorder(clpre, Loading)
clpre
# Some loading on Personal Moral Norm (0.64)

# Perceived Response Costs
## Intercorrelation -> none, only one indicator
icprc <- evalmm %>% filter(Construct == "Perceived Response Costs")
## cross-loadings
clprc <- data.frame(Indicator = character(),
                       Construct = character(),
                       Loading = double ())
for (i in 1:length(icprc$Indicator)){
  x <- icprc$Indicator[i]
  clprc <- clprc %>% rbind(data.frame(Indicator =  {{x}},
                                            Construct = names(sumfs$validity$cross_loadings[{{x}},]),
                                            Loading = sumfs$validity$cross_loadings[{{x}},]))
  
}
data.table::setorder(clprc, Loading)
clprc
```
...
There are no issues with collinearity as measured with VIF, and all indicators are significant and relevant. Convergent validity is established for all constructs.
...
## Collinearity
The variance inflation factor (VIF) should be < 5, ideally $\seq\ 3.
...
# Results summary
The reflective measurement model and the formative higher-order constructs meet all quality criteria. The formative measurement model shows HTMT problems. The HOC model met all quality criteria.

---
title: "Climate Crisis Model Iteration 2-c-3-b-1 (Driving) Structural Model Evaluation"
...
model <- readRDS("Models/model-cc-2-c-3-b-1.RDS")
bootmodel <- readRDS("Models/model-boot-cc-2-c-3-b-1.RDS")
bootfsmodel <- readRDS("Models/model-fs-boot-cc-2-c-3-b-1.RDS")
proxymodel <- readRDS("Models/model-proxy-cc-2-c-3-b-1.RDS")
summo <- summary(model)
...
```
There is no collinearity as indicated by VIF.
...
In particular, there is no discernible effect of Benevolence on any of the descendent constructs or Competence on Perceived Self-Efficacy and Perceived Response Costs.
...
oospp <- (as.data.frame(sumplspre$PLS_out_of_sample) %>%
  ...
  cbind(`Prediction Errors` = c("PLS RMSE", "PLS MAD", "LM RMSE", "LM MAD", "RMSE Difference", "MAD Difference")))[c(1,3,5,2,4,6),c(5,1:4)]
datatable(oospp,
...
```
For the approximation model, only CCBI3 is predicted better by the PLS model.
...
```
Competence has no significant influence on either Perceived Response Costs or Perceived Self-Efficacy, and Benevolence none on Perceived Response Costs.
...
# Results summary
Looking only at higher-order and regular constructs, all path coefficients are significant and effect size are sufficiently large.
However, both of the Distrusting Beliefs lower-order constructs have no discernible effect on or path to Perceived Responce Costs.
Therefore, this relationship should be tentatively removed to evaluate wether that improves model performance.

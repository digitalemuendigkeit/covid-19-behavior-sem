---
title: "Climate Crisis Model Iteration 2-d-2 (General) Structural Model Evaluation"
...
Some VIF values are not available.
...
Other than that, all R²s are between 0.1 and 0.25, indicating very weak in-sample predictive power.
...
f² indicates negligible effect sizes for Subjective Norm's lower-order-constructs, but not Subjective Norm itself.
...
```
For the approximation model, not a single indicator predicted better by the PLS model than by the linear model in terms of both RMSE and MAD.
...
# Results summary
There is nothing to optimize in the model, but overall, predictive power is very low.

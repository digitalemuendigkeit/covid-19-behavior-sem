# This file generates figures for our paper using the external data and
# our final data (with missing data removed)
...
agge <- data.frame(read_csv2("external-data/age-gender-germany.csv", skip = 8))[-c(87:90),-1]
agge$X2 <- c(0:85)
agge[agge == "-"] <- NA
colnames(agge)[1] <- "Age"
...
nekge <-  data.frame(read_csv2("external-data/household-income-classes.csv", skip = 7, skip_empty_rows = TRUE, ))[-c(4:6),]
...
# Source: Statistisches Bundesamt, Haushaltsbuch, 2019
negge <-  data.frame(read_csv2("external-data/household-income-size.csv", skip = 7, skip_empty_rows = TRUE, ))[-c(4:6),-c(1:3)]
